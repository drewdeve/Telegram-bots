...
from messagespay import MESSAGES
from configforpay import BOT_TOKEN, PAYMENTS_PROVIDER_TOKEN, TIME_MACHINE_IMAGE_URL
...
PRICES = [
    types.LabeledPrice(label='The Real Time Machnine', amount=4200000),
    types.LabeledPrice(label='Gift packaging', amount=30000)
]

TELEPORTER_SHIPPING_OPTION = types.ShippingOption(
    id='teleporter',
    title='World* teleport'
).add(types.LabeledPrice('Teleport', 1000000))

RUSSIAN_POST_SHIPPING_OPTION = types.ShippingOption(
    id='ru_post', title='Russian Post')
RUSSIAN_POST_SHIPPING_OPTION.add(
    types.LabeledPrice(
        'Wooden box with shock absorbing suspension inside', 100000)
)
RUSSIAN_POST_SHIPPING_OPTION.add(
    types.LabeledPrice('Urgent departure (5-10 days)', 500000)
)

PICKUP_SHIPPING_OPTION = types.ShippingOption(id='pickup', title='Pickup')
PICKUP_SHIPPING_OPTION.add(types.LabeledPrice('Pickup in Moscow', 50000))
...
async def process_shipping_query(shipping_query: types.ShippingQuery):
    ...
    if shipping_query.shipping_address.country_code == 'RU':
        ...
        if shipping_query.shipping_address.city == 'Moscow':
